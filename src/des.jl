...
function blockoperate(plaintext::BitVector, key::BitVector, operation::Op)
    ...
    ipblock = permute(plaintext, IP_TABLE)
    pc1key = permute(key, PC1_TABLE)
    for round in 1:16
        expansionblock = permute(ipblock[BLOCKSIZE÷2+1:end], EXPANSION_TABLE)
        if operation == encrypt
            pc1key = rol(pc1key)
        ...
        subkey = permute(pc1key, PC2_TABLE)
        if operation == decrypt
            pc1key = ror(pc1key)
...
const IP_TABLE = [  # Initial permutation
...
const FP_TABLE = [  # Final permutation
